...
void Camera::initCamera(){
	...
	upAngle = 0.0;
	fovy = 45.0;
	aspect = 16.0/10.0;  //16 : 10分辨率
	scale = 1.5;
	zNear = 0.01;
	zFar = 100.0;
	eye = glm::vec4(0.3 * radius, 5.0, 20.0, 1.0);
	at = glm::vec4(0.0, 0.0, 0.0, 1.0);
	up = glm::vec4(0.0, 1.0, 0.0, 0.0);
	pitch = 0.0f, yaw = 150.0f, upAngle = 15.0f;
...
void Camera::keyboard(GLFWwindow* window)
{
	...
	// CTRL和ALT控制相机上升和下降
	int CTRL = glfwGetKey(window, GLFW_KEY_LEFT_CONTROL), ALT = glfwGetKey(window, GLFW_KEY_LEFT_ALT);
	...
	else if (CTRL == GLFW_PRESS || CTRL == GLFW_REPEAT) {
		eye += cameraSpeed * up;
	}
	else if (ALT == GLFW_PRESS || ALT == GLFW_REPEAT) {
		eye -= cameraSpeed * up;
	}
	//空气墙
	eye.x = std::min(25.f, eye.x); 
	eye.x = std::max(-25.f, eye.x);
	eye.y = std::min(35.00f, eye.y);
	eye.y = std::max(0.1f, eye.y);
	eye.z = std::min(25.f, eye.z);
	eye.z = std::max(-25.f, eye.z);
}
